...
export class CreateVehicleTypeComponent implements OnInit {
  ...
  constructor(private serviceVehicleType: VehicleTypeService, private formBuilder: FormBuilder, private toast: ToastrService) { }
  ...
  clickSubmit() {
    ...
    this.serviceVehicleType.addEntity(vehicleType).subscribe(newVehicleType => this.createVehicleType.next(newVehicleType), _ => this.toast.error('Не вдалось створити тип транспорт', 'Помилка створення нового типу транспорту'));
